...
module kyber_main_fsm (
    ...
    wire signed [15:0] ntt_ext_dout_b; // â†? Má»šI: há»‡ sá»' táº¡i (ext_addr+1), dÃ¹ng cho PWMA Ä'á»?c s[2k+1]
    ...
    // Ä?á»?c Twiddle Factor cho PWMA (Xá»­ lÃ½ Item #17)
    ...
    // --- 3.6 Comparator & 3.7 Compressor (Giá»¯ nguyÃªn) ---
    ...
    // PWMA dÃ¹ng cáº£ 2 port cá»§a Main RAM Ä'á»ƒ Ä'á»?c/ghi cáº·p t[2k], t[2k+1]
    assign we_a   = (mux_sel == 3'd1) ? cbd_we           : (mux_sel == 3'd2) ? pwma_ram1_we     : fsm_we_a;
    assign we_b   = (mux_sel == 3'd2) ? pwma_ram1_we     : fsm_we_b;
    
    // t_i náº±m á»Ÿ Main RAM, má»-i há»‡ sá»' 16-bit Ä'á»™c láº­p:
    // PWMA map pair index k -> Ä'á»‹a chá»‰ (2k, 2k+1)
    ...
    // A matrix cache (offset 0..1023 cho k=2): cho phÃ©p PWMA Ä'á»?c cáº·p A[2k], A[2k+1]
    ...
    always @(*) begin
        if (ntt_mux_sel == 3'd1) begin
            // CBD â†' NTT RAM (Ä'á»‹a chá»‰ 1:1)
            ntt_ext_we = cbd_we; ntt_ext_addr = cbd_ram_addr[7:0]; ntt_ext_din = cbd_poly_out;
        end else if (ntt_mux_sel == 3'd2) begin
            // PWMA Ä'á»?c s[j]: phÃ¡t Ä'á»‹a chá»‰ CHáº´N = 2 * pwma_ram0_addr
            // â†' NTT RAM port A: s[2k]   = ntt_ext_dout
            // â†' NTT RAM port B: s[2k+1] = ntt_ext_dout_b  (addr tá»± Ä'á»™ng = 2k+1)
            ntt_ext_we = 1'b0;
            ntt_ext_addr = {pwma_ram0_addr[6:0], 1'b0}; // 2 * cnt, cháºµn
            ntt_ext_din = 16'd0;
        end else begin
    ...
    always @(*) begin
        ...
        // s[j][2k+1] = ntt_ext_dout_b (NTT RAM port B, addr = 2k+1) â†? FIX
        ...
        // A[i,j][2k], A[i,j][2k+1] láº¥y tá»« A cache dual-port
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            case (state)
                ...
                // ===========================================================
                // VÃ'NG Láº¶P E: Sinh e[i] -> NTT -> RAM offset 0x000 (LÃ  t ban Ä'áº§u)
                // ===========================================================
                S_KG_PRF_E: begin
                    hash_cmd <= 3'd1; hash_bytes <= 8'd33; hash_prf_eta3 <= eta1_is_3; // e dÃ¹ng eta1 (giá»'ng s)
                ...
                S_KG_CBD_E: begin
                    ntt_mux_sel <= 3'd1; cbd_base_addr <= 11'd0; cbd_eta3_mode <= eta1_is_3; // e dÃ¹ng eta1
                ...
                S_KG_DMA_E: begin
                    ...
                    if (dma_cnt > 1) begin                                           // â†? >0 â†' >1
                        fsm_we_a <= 1;
                        fsm_addr_a <= (loop_i * 11'd256) + dma_cnt[10:0] - 11'd2;   // â†? -1 â†' -2
                        fsm_din_a  <= ntt_ext_dout;
                    end
                    if (dma_cnt == 12'd257) begin                                    // â†? 256 â†' 257
                        dma_cnt <= 0;
                ...
                // ===========================================================
                // VÃ'NG Láº¶P S: Sinh s[i] -> NTT -> RAM offset 0x200 (Báº£o lÆ°u SK)
                // ===========================================================
                S_KG_PRF_S: begin
                    hash_cmd <= 3'd1; hash_bytes <= 8'd33; hash_prf_eta3 <= eta1_is_3;
                    hash_din <= { 824'd0, {5'd0, loop_j}, flip_bytes_32(reg_sigma) }; // â†? loop_j
                    if (!hash_sent) begin hash_start <= 1; hash_sent <= 1; end
                    else if (!hash_busy && !hash_done) begin
                        hash_sent <= 1'b0;
                    end
                ...
                S_KG_DMA_S: begin
                    mux_sel <= 3'd0; ntt_mux_sel <= 3'd0;
                    fsm_ntt_addr <= dma_cnt[7:0];
                    if (dma_cnt > 1) begin                                           // â†? Ä'á»•i >0 thÃ nh >1
                        fsm_we_a   <= 1;
                        fsm_addr_a <= sk_ram_base + ({9'd0, loop_j} * 12'd256) + dma_cnt[11:0] - 12'd2; // s[j] after t region
                        fsm_din_a  <= ntt_ext_dout;
                    end
                    if (dma_cnt == 12'd257) begin                                    // â†? Ä'á»•i 256 thÃ nh 257
                        dma_cnt    <= 0;
                        loop_i     <= 0;
                        pwma_sent  <= 0;
                        state      <= S_KG_PWMA_A_S;
                ...
                // ===========================================================
                // Táº O MA TRáº¬N A: 1 láº§n duy nháº¥t cho toÃ n bá»™ KxK
                // ===========================================================
                S_KG_GEN_A: begin
                    mux_sel <= 3'd3;
                    if (!gm_sent) begin gm_start <= 1; gm_sent <= 1; end
                    if (gm_done) begin
                        loop_j    <= 0;          // â†? THÃŠM: reset loop_j
                        loop_i    <= 0;
                        hash_sent <= 0;
                        state     <= S_KG_PRF_S; // â†? Báº¯t Ä'áº§u vÃ²ng láº·p s
                    end
                end
                ...
                // S_KG_LOAD_S: begin
                //     // Ä?á»?c s[loop_j] tá»« Main RAM (Port B), náº¡p láº¡i vÃ o bá»¥ng NTT Core
                //     mux_sel <= 3'd0; ntt_mux_sel <= 3'd0;
                ...
                S_KG_PWMA_A_S: begin
                    ...
                    if (pwma_done) begin
                        pwma_sent <= 0;
                        if (loop_i == (k_val - 3'd1)) begin
                            // Xong táº¥t cáº£ hÃ ng i cho cá»™t j nÃ y
                            if (loop_j == (k_val - 3'd1)) begin
                                // Xong toÃ n bá»™ A*s+e: chuyá»ƒn sang pack bytes chuáº©n
                                pack_phase <= 2'd0;
                                pack_poly  <= 3'd0;
                                pack_pair  <= 8'd0;
                                pack_step  <= 3'd0;
                                ...
                                loop_j    <= loop_j + 1;   // Sang cá»™t s tiáº¿p theo
                                loop_i    <= 0;
                                hash_sent <= 0;
                                state     <= S_KG_PRF_S;   // Sinh s[j+1]
                            end
                        end else begin
                            loop_i <= loop_i + 1;          // Sang hÃ ng tiáº¿p theo
                            // KhÃ´ng cáº§n reset pwma_sent, Ä'Ã£ set = 0 á»Ÿ trÃªn
                        end
                    end
                end
